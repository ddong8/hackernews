<template>
  <div class="item-view view">
    <div class="item-view-header">
      ...
      <p class="meta">
        {{ item.points }} {{ $t('article_action.points') }} |
        <router-link :to="$i18n.path('user/' + item.user)">
          {{ item.user }}
        </router-link>
        {{ $t('article_action.by') }}
    ...
    <div class="item-view-comments">
      <lazy-wrapper :loading="item.loading">
        <p class="item-view-comments-header">
          {{ item.comments.length > 0 ? item.comments.length + ' ' + $t('article_action.comments') : $t('article_action.no-comments') + '.' }}
        </p>
